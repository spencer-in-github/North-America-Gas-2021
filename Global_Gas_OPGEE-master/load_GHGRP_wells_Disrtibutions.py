...
wells_path = os.path.join(cwd, 'EF_W_ONSHORE_WELLS_2020.csv')
wells_data = pd.read_csv(wells_path, skiprows=1, usecols=[1, 6],
                          names=['FACILITY_ID', 'WELL_ID_NUMBER'],
                          index_col=None,
                          encoding_errors='ignore')

# Enverus_path = os.path.join(cwd, 'annualDF_2019_AllUSA_22717.csv')
# Enverus_data = pd.read_csv(Enverus_path, skiprows=1, usecols=[1, 2, 3],
#                           names=['API/UWI', 'Annual Oil [bbl/year]', 'Annual Gas [mscf/year]'],
#                           index_col=None,
#                           encoding_errors='ignore')

Enverus_path = os.path.join(cwd, 'annualDF_2020_SpatialJoin_2258_v2.csv')
...
wells_data.drop(11511, axis = 0, inplace = True)
wells_data.drop(491828, axis = 0, inplace = True)
wells_data.drop(442702, axis = 0, inplace = True)
wells_data.drop(153287, axis = 0, inplace = True)
wells_data.drop(341405, axis = 0, inplace = True)
wells_data.drop(347146, axis = 0, inplace = True)
wells_data.drop(414510, axis = 0, inplace = True)
...
for idx, row in tqdm(wells_data.iterrows()):
    ...
    csvPath = os.path.join(cwd, 'API_Facility_correspondence_2020.csv')
