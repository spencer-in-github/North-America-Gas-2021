...
# Compute row sums
col_sums = flow_matrix_square.sum(axis=1)

# Normalize each row: divide each value by the row's sum
normalized_matrix = flow_matrix_square.div(col_sums, axis=0)

# Fill any NaNs (from divide-by-zero) with 0
...
# Optionally save to CSV
normalized_path = path_save + "normalized_input_matrix.csv"
normalized_matrix.to_csv(normalized_path)
print(f"Saved normalized matrix to: {normalized_path}")
...
# Convert to NumPy array and remove last row and column
# Last row/column is international
# International requires no input from other states (lack of data) therefore gives a zero column
# would cause matrix to be not invertible (cant compute inverse)
X_mat = X.values[:-1, :-1]

# Normalize with row sum again after removing international column/row
# Compute row sums
row_sums = X_mat.sum(axis=1, keepdims=True)
# Avoid divide-by-zero: set zeros to 1 temporarily
row_sums[row_sums == 0] = 1
# Normalize rows manually
X_mat = X_mat / row_sums
# Replace NaNs or infs just in case (e.g., from divide-by-zero)
X_mat = np.nan_to_num(X_mat)

# Identity matrix of compatible size
I = np.identity(X_mat.shape[0])

# Compute (I - X)
I_minus_X = I - X_mat

np.linalg.det(I_minus_X)
...
normalized_cols = {col: normalize(df[col].values) for col in df.columns}

# Check cosine similarity (dot product of normalized vectors)
parallel_pairs = []
columns = list(normalized_cols.keys())

for i in range(len(columns)):
    for j in range(i + 1, len(columns)):
        col_i = columns[i]
        col_j = columns[j]
        dot_product = np.dot(normalized_cols[col_i], normalized_cols[col_j])
        if np.isclose(abs(dot_product), 1.0):  # parallel if cosine = ±1
            parallel_pairs.append((col_i, col_j))

print("Parallel column pairs:", parallel_pairs)

# Attempt matrix inversion
try:
    epsilon = 1e-6
    I_minus_A_reg = I_minus_X + np.eye(I_minus_X.shape[0]) * epsilon
    inverse = np.linalg.inv(I_minus_A_reg)
    # Convert back to DataFrame with same index/columns
    inverse_df = pd.DataFrame(inverse, index=X.index[:-1], columns=X.columns[:-1])

    cleaned_matrix = inverse_df.mask(inverse_df < 1e-8, 0)

    # Save to CSV
    inverse_df.to_csv(path_save + "inverse_I_minus_X.csv")
    cleaned_matrix.to_csv(path_save + "cleaned_leontief_inverse.csv")
    print("Inverse matrix saved to " + path_save + "inverse_I_minus_X.csv")
    print(inverse_df.head())
except np.linalg.LinAlgError as e:
    print("Matrix (I - X) is not invertible:", e)
